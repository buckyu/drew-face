...
#define GET_PIXEL_OF_MATRIX(MTX,X,Y,CHANNEL) image.at<cv::Vec<unsigned __int8, 4>>(Y,X)[CHANNEL]
#define WIDTH image.cols
#define HEIGHT image.rows

    
    unsigned __int8 *testimagedataMod1 = (unsigned __int8*)malloc(HEIGHT * WIDTH *4);
    unsigned __int8 *testimagedataMod2 = (unsigned __int8*)malloc(HEIGHT * WIDTH *4);
    ...
    for(int x = 0; x < WIDTH; x++) {
        for(int y = 0; y < HEIGHT; y++) {
            ...
            unsigned __int8 pxR = GET_PIXEL_OF_MATRIX(image, x, y, 0);
            unsigned __int8 pxG = GET_PIXEL_OF_MATRIX(image, x, y, 1);
            unsigned __int8 pxB = GET_PIXEL_OF_MATRIX(image, x, y, 2);
